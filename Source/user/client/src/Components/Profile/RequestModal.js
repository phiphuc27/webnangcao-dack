...
import { acceptRequest, declineRequest } from '../../Actions/contract';

const RequestModal = ({ request, show, onHide }) => {
  ...
  const onAccept = () => {
    const data = {
      ...
      value: {
        IDDK: ID,
        NGAYBD: moment().format('YYYY-MM-DD'),
        NGAYKT: moment(NGAYKT).format('YYYY-MM-DD'),
        TRANGTHAI: 0
      }
    };
    ...
    >
      ...
      {TRANGTHAI === 0 && (
        <Modal.Footer>
          ...
          <Button onClick={onAccept}>Chấp nhận</Button>
        </Modal.Footer>
      )}
    </Modal>
  );
